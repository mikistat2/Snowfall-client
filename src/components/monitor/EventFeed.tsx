  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 space-y-1.5 overflow-y-auto pr-1">
        {events.map((e) => (
          <div key={e.id} className="flex items-center gap-2.5 rounded-lg bg-surface px-3 py-2 shadow-sm">
            <span className={`h-2.5 w-2.5 shrink-0 rounded-full ${severityDot[e.severity]}`} />
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm">{e.message}</p>
              <p className="text-xs text-fg-subtle">{new Date(e.created_at).toLocaleTimeString()}</p>
            </div>
        ...
        {events.length === 0 && <p className="text-sm text-fg-subtle">—</p>}
      </div>
    </div>
  );
}
